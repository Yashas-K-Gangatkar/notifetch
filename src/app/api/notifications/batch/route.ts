...
export async function POST(request: NextRequest) {
  try {
    ...
    // Using Prisma 6.x createManyAndReturn to reduce DB roundtrips from O(n) to O(1)
    const created = await db.notification.createManyAndReturn({
      data: notifications.map((n: Record<string, any>) => ({
        userId,
        ...
        receivedAt: (n.receivedAt && !isNaN(Date.parse(n.receivedAt as string)))
